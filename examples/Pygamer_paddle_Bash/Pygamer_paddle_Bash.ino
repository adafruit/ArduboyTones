...
ArduboyTones sound(returnTrue);        // creates sound object for arduboy tone player
extern Adafruit_Arcada arcada;         // create arcada object so we can control the display, inputs, sound, etc.

    // sound melodies
const uint16_t titleMelody[] PROGMEM = {
    NOTE_D5, 100, NOTE_D6, 100, NOTE_C6, 100, NOTE_B5, 500,
    NOTE_A5, 100, NOTE_G5, 100, NOTE_FS5, 100, NOTE_E5, 500,
    NOTE_G4, 100, NOTE_D5, 100, NOTE_B5, 100, NOTE_A5, 500,
    NOTE_FS5, 100, NOTE_G5, 2000,
    TONES_END
};

const uint16_t playerWinsMelody[] PROGMEM = {
    NOTE_D6, 100, NOTE_REST, 67, NOTE_B6, 167, NOTE_A6, 167,
    NOTE_G6, 150, NOTE_D7, 250, NOTE_REST, 250, NOTE_E7, 500, NOTE_D7, 500,
    TONES_END
};

const uint16_t enemyWinsMelody[] PROGMEM = {
    NOTE_D7, 100, NOTE_B6, 100, NOTE_AS6, 100, NOTE_F6, 500,
    NOTE_B6, 100, NOTE_AS6, 100, NOTE_F6, 100, NOTE_D6, 500,
    NOTE_AS6, 100, NOTE_F6, 100, NOTE_D6, 100, NOTE_B5, 500,
    NOTE_F6, 100, NOTE_D6, 100, NOTE_B5, 100, NOTE_F5, 1000,
    TONES_END
};
...
void setup() {
    ...
    sound.volumeMode(VOLUME_IN_TONE);           // allows us to control volume within sound functions
    ...
    //---------------------------VOID LOOP-----------------------//
void loop() {                           // I tried to list these mostly showing how the enumerators flow
...
void endGame()      // runs someone reaches the max score
{
    ...
    //------------------SOUND FUNCTIONS-----------------------//
void bounceSound() {  // for ball collisions
    sound.tone(NOTE_D7,60);
}

void playerScoreSound() {
    sound.tone(NOTE_G6,100, NOTE_B6,100, NOTE_D7,100);
}

void enemyScoreSound() {
    sound.tone(NOTE_D7,100, NOTE_B6,100, NOTE_GS6,100);
}

void playerWinsSound() {
    sound.tones(playerWinsMelody);
}

void enemyWinsSound() {
    sound.tones(enemyWinsMelody);
}

void titleSound() {
    sound.tones(titleMelody);
}

bool returnTrue() {   // allows us to force the arduboytones class by making it think arduboy sound is activated
    return true;
}
